...
public class PlayFabAuthService
{
    ...
    public bool HasUserPreviouslyLoggedIn
    {
        get
        {
            return PlayerPrefs.HasKey(_LoginRememberKey) && PlayerPrefs.HasKey(_PlayFabRememberMeIdKey);
        }
    }
    ...
    public void LinkGoogleAccount()
    {
        ...
        {
            Debug.Log("Google Account successfully linked");
        }, (error) =>
        {
            //report errror back to the subscriber
            if (OnPlayFabError != null)
            {
                OnPlayFabError.Invoke(error);
            }
